...
//Prove this using SumOne
{
  ...
  if(s==[]){
    ...
    calc == {
      ...
      {sums(s[..|s|-1]);}
      SumS(s[..|s|-1]) + s[|s|-1];
      SumM(multiset(s[..|s|-1])) + s[|s|-1];
      { 
        assert s[..|s|-1] + [s[|s|-1]] == s; 
        assert multiset(s[..|s|-1]) == multiset(s) - multiset{s[|s|-1]};
      }
      SumM(multiset(s) - multiset{s[|s|-1]}) + s[|s|-1];
      {
...
//Prove this using SumByParts
{
  if(v.Length == 0){
    ...
    calc =={
      ...
      SumM(multiset(v[c..f]));
      { 
        assert [v[c]] + v[c+1..f] == v[c..f]; 
        assert multiset(v[c..f]) == multiset(v[c+1..f]) + multiset{v[c]};
      }
      SumM(multiset(v[c+1..f])+multiset{v[c]});
      {SumByParts(multiset(v[c+1..f]), multiset{v[c]});}
      SumM(multiset(v[c+1..f])) + v[c];
      SumVM(v,c+1,f) + v[c];
    }
  }
}
...
{ArrayFactsM<int>();
 ...
 while (i<m) //First loop computes the sum [0..m)
   decreases m-i//Write 
   invariant 0<=i<=m
   invariant sum==SumVM(v, 0,i)//write
 {
  sum:=sum+v[i];
  i:=i+1;
  }
